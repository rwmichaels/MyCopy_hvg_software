...
public class HVmainMenu extends JFrame implements ActionListener, ItemListener {
    ...
    public Vector hvmap = new Vector(); // calorimeter map    
    ...
    /**
     * Initialization of HVframes by calling initHVF() method and creation 
	...
	for (Enumeration e = frameList.elements() ; e.hasMoreElements() ;) {
	    ...
	    hostname = frameList.getToken(1,lhs);
	    try {
		portnum =  Integer.parseInt(frameList.getToken(2,lhs));
	    
	    String protocol = frameList.getToken(3,lhs);
	    String login = frameList.getToken(4,lhs);
	    String password = frameList.getToken(5,lhs);
...
//	menuItem.setEnabled(false);
	...
	for (Enumeration f = hvft.elements() ; f.hasMoreElements() ;) {
	...
	helpframe.setLocation(screenSize.width/2 - demo.INITIAL_WIDTH/2,
			      screenSize.height/2 - demo.INITIAL_HEIGHT/2);
	helpframe.setSize(demo.INITIAL_WIDTH, demo.INITIAL_HEIGHT);
	helpframe.show();	
...
//*** 03-Sep-05: we have sumbodule here in form S0,.., so add it ro module slot number
	...
	if (e.getActionCommand().equals("map_config")) {	
	    mapDialog.initialize(this, "HV Map List",
                              "HV Map Parameters");
	    String selectedName = mapDialog.showDialog(null,
						       "null");
	...
	if (e.getActionCommand().equals("add/remove")) {	
	    listDialog.initialize(this, "HV Host List",
    ...
    */
     ...
     * @return none
     */
    public void updateF(final HVframe f, final Vector v, final String id) {
	final hvtools.SwingWorker worker = new hvtools.SwingWorker() {
		public Object construct() {
		    int l = v.size();
		    for(int i=0; i<l;i++) {
