/* Last update: 1 Jun 2005: Add sleep_time constant 
 * 
 */
package socket;
 ...
 * @version 0.3 02/14/02 - replaced wait("Password:",-1) to  wait("passwd:",-1) and
 *                         sendLine() to send()        in method login();
 * @see TelnetIO
 */
public class TelnetWrapper
{
  ...
  /** The current prompt for login on the remote system. */
    private String loginPrompt; // r.p. add 21-Feb-2003
  /** The current prompt for password on the remote system. */
    private String passwordPrompt; // r.p. add 21-Feb-2003
  ...
  /** The default logni prompt used by all TelnetWrappers unless specifically
   * overridden.
   * @see #setLoginPrompt
   */
    private static String defaultLoginiPrompt = "login:";// r.p. add 21-Feb-2003

  /** The default password prompt used by all TelnetWrappers unless specifically
   * overridden.
   * @see #setPasswordPrompt
   */
    private static String defaultPasswordPrompt = "passwd:";// r.p. add 21-Feb-2003
  ...
  private static String defaultPassword = null;
    ...
    private static long sleep_time = 5; // 1 Jun 2005: sleep interval in 'wait' function
   ...
   * @see #setPrompt
   */
    public String sendLine(String command, long timeout) 
	throws IOException, TimedOutException
    {
	if(command.charAt(command.length() -1) != '\r') 
	    command += "\r";
	send(command);
	String s = receiveUntil(prompt, timeout);
  ...
  public void login(String loginName, String password) throws IOException
  {
    wait(loginPrompt);
    send(loginName + "\r");
    wait(passwordPrompt); // r.p. modd 21-Feb-2003
    //    wait("Password:");
    send(password + "\r");
  }

  /** Logs in as a particular user and password and set timeout. 
    * Returns after receiving prompt. */
  public void login(String loginName, String password, int timeout) throws TimedOutException,IOException
  {
    wait(loginPrompt,timeout);
    send(loginName + "\r");
    wait(passwordPrompt, timeout); // r.p. modd 21-Feb-2003
    //    wait("Password:");
    sendLine(password + "\r");
  }
  ...
  public TelnetWrapper(String host) throws IOException
  {
    ...
    setLoginPrompt(loginPrompt);// r.p. add 21-Feb-2003
    setPasswordPrompt(passwordPrompt);// r.p. add 21-Feb-2003
    tio.connect(host);
  ...
  /** Sets the expected login prompt. 
   * If this function is not explicitly called, the default login prompt is used.
   * 
   */
  public void setLoginPrompt(String prompt)
  {
      //    if(prompt == null) throw new IllegalArgumentException("null prompt.");
    this.loginPrompt = prompt;
  }

  /** Sets the expected password prompt. 
   * If this function is not explicitly called, the default password prompt is used.
   * 
   */
